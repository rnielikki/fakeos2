...
export let showDialogIfError = function(message:FileEditResult, fileName:string, targetFileName?:string) {
    switch(message) {
        ...
        case FileEditResult.Immutable:
            WindowFactory.OpenDialog(null, "LOCKED File", `File ${NameOrName(fileName, targetFileName)} is not edible.\nI mean, not editable.`)
            break;
        ...
        case FileEditResult.InvalindName:
            WindowFactory.OpenDialog(null, "INVALID Name", `${fileName} is invalid.\nFile name should not be: empty, contains slash(/).`)
            break;
